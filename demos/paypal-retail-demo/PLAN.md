...
Current phase: **Milestone 11.5 Buyer Flow Interaction Recovery is queued before continuing Milestone 13. The gap audit found several checked UI milestones where the visual shell exists but buyer actions are not fully wired: PDP/cart/minicart actions, one-expanded-section checkout accordion progression, editable checkout fields, Pickup guest ZIP/postcode to ranked store-list modal, Pickup logged-in preselected store with Change store modal, partial-inventory Order Summary updates, payment-method selection, and selected payment surface switching. The UX review also found the POP MART profile too restrained for a blind-box collectible storefront, so a playful premium collectible visual refresh is queued for Milestone 16: multi-accent tokens, rounded/tactile product surfaces, sticker-like badges, drop-calendar styling, collectible promo cards, and calmer branded checkout accents. Complete M11.5 first, then resume Milestone 13 synchronized PayPal shipping callback totals, final express review snapshot, confirm-triggered capture, and amount consistency guard enforcement. Broader responsive screenshot QA and live sandbox capture/wallet checks remain open. Local Supabase migration verification remains blocked until Docker is available**.
