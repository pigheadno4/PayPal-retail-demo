...
2026-07-07 update: Checkout Round 3 pickup and drawer repair is the active next Cart/Checkout slice after the user approved Option A. Use `ROUND3_CHECKOUT_PICKUP_DRAWER_PLAN.md` as the implementation handoff and update tracking task-by-task. The scope is pickup store-picker/header layering, picker cancel fallback to Pickup location, mobile duplicated order-detail removal through a single bottom drawer, billing-submit optimistic progression, real backend-backed promo activation, selected PayPal/Pay Later action width parity, and a repeatable Round 3 evidence helper.
