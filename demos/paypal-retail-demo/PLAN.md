...
Current phase: **Milestone 13.1 cart, checkout draft, and Pay Later recovery on `main`. The merged baseline now includes checkout payment-action gating, Delivery checkout live-data recovery, server checkout-draft UUID creation, retryable section errors, and official PDP/cart/minicart express SDK surfaces. The latest live audit found the cart binding/header contract is the first blocker: the browser has fixture cart data but no persisted cart client secret, so cart, checkout draft, and PayPal create-order calls cannot send the required `x-cart-id` plus `x-cart-secret` headers. Before confirm-triggered capture resumes, stabilize browser cart binding persistence, fresh-browser guest cart bootstrap into server cart rows, minicart quantity controls, checkout route cart continuity, server-ready create-order gating, Pay Later SDK v6 eligibility/detail gating, Pickup guest/logged-in initial state separation, and merchant-visible PayPal create-order failure feedback. The starter-cart bootstrap is a tactical bridge until the buyer UI loads products and carts directly from the server catalog. After M13.1 verification, continue Milestone 13 Review and Confirm capture plus amount-guard enforcement. The playful POP MART visual refresh remains queued for Milestone 16. Broader responsive screenshot QA and live sandbox capture/wallet checks remain open. Local Supabase migration verification remains blocked until Docker is available**.
