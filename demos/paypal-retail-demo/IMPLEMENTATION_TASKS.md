...
- [ ] Apply the Cart + Checkout Round 2 density and mobile checkout disclosure refinement from `CART_CHECKOUT_A_PLUS_SPEC.md`: compact cart/title typography, hidden native quantity spinners, form-first mobile checkout, real Delivery/Pickup switch, complete shipping address collection, shipping method section, truthful promo UX, and v5 grabber-only order-details sheet.
  ...
  - [x] Round 2 UI/UX caveat review gate: reconcile the later `ui-ux-pro-max` sub-agent verification verdict `Ready with caveats` into the deploy-quality checklist.
    ...
    - Follow-up evidence: a second read-only `ui-ux-pro-max` sub-agent verification returned `Ready with caveats`: visible Round 2 polish matches the approved virtual mockups. Later local evidence closed the selected-provider browser request/callback caveat for PayPal and Pay Later, the API-backed real discount visual proof with `-$8.55 promo (AUTO10 + BUNDLE8)` in sticky, summary, and Sheet surfaces, selected Pay Later timeout-warning triage, local sticky/fixed overlap for the recorded required targets, and helper-owned recalculation/failure visual rows. Deploy closure still needs hosted smoke and hosted/required-state sticky/fixed overlap proof with focused inputs, Sheet controls, footer, PayPal messages, dialogs, and skip-link layering.
  ...
  - [ ] Round 2 final verification gate: run focused Cart/Checkout/App/style tests, `npm run typecheck`, `npm run lint`, `npm run format:check`, `git diff --check`, and GUI/browser evidence for cart plus mobile checkout initial, saved, payment-ready, selected-payment, expanded-sheet, collapsed-sheet, and failure/recalculation states.
    ...
    - [ ] Hosted post-deploy smoke evidence: current Render health returned `200` with debug ID `dbg_ba8fd084a906`, but the deployed index still served `/assets/index-CuNnoNV5.js` and `/assets/index-CajkqOhp.css` before this slice was deployed. The new hosted smoke against that current deployed build failed on `cart-first-pass-320` due a cart-restore fetch console error and on `checkout-footer-clearance-390` with `sticky occluded 7 independent targets`. Rerun `npm run evidence:round2:hosted-smoke` against Render after this commit deploys; closure requires `failedRows: []` and route-scoped console/sticky evidence on the deployed asset.
