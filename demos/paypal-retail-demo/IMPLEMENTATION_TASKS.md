...
- [ ] Continue Milestone 16 with Checkout Round 3 pickup and drawer repair using `ROUND3_CHECKOUT_PICKUP_DRAWER_PLAN.md` as the execution handoff and `CART_CHECKOUT_A_PLUS_SPEC.md` as the UX contract.
  ...
  - [x] Round 3 billing progression local implementation slice: Delivery billing opens the Shipping options/method section within 250ms after client validation; Pickup billing opens Pickup date within 250ms; pending totals/recalculating state is preserved while the draft update settles; provider actions remain disabled until latest draft totals are current; and slow-save failures return focus/error to Billing.
    - Evidence: failing-first interaction tests covered slow Delivery billing, slow Pickup billing, literal focus return to Billing, Delivery billing failure rollback, and Pickup billing failure rollback. Focused verification passed `npm test -- web/src/features/checkout/CheckoutPage.interactions.test.tsx` (`24` tests) and `npm test -- web/src/features/checkout/CheckoutPage.interactions.test.tsx web/src/features/checkout/CheckoutPage.test.tsx` (`60` tests). Browser/API-backed timing and visual proof remains open in the Round 3 evidence helper.
    - Inspection standard: interaction tests simulate slow and failed billing updates for Delivery and Pickup and record timing, active section, readiness state, and displayed total/promo/shipping/tax labels.
  ...
  - [ ] Round 3 evidence helper and docs closure: add `tools/round3-checkout-pickup-drawer-evidence.playwright.js` plus `npm run evidence:round3:checkout-pickup-drawer`; update `CART_CHECKOUT_A_PLUS_SPEC.md`, `DESIGN.md`, `IMPLEMENTATION_TASKS.md`, `tracking/todos.md`, `tracking/test-cases.md`, and `tracking/progress.md` with evidence paths and remaining hosted caveats.
    - Evidence: helper and npm script are now added, and `server/tests/evidenceScripts.test.ts` statically locks the required command, row IDs, and metric names. Verification passed `npm test -- server/tests/evidenceScripts.test.ts`, `npm run lint`, and `npm run typecheck`. Full API-backed helper execution remains open until a configured local or hosted app is available.
