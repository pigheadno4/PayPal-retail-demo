...
- [ ] Continue Milestone 16 with Checkout Round 3 pickup and drawer repair using `ROUND3_CHECKOUT_PICKUP_DRAWER_PLAN.md` as the execution handoff and `CART_CHECKOUT_A_PLUS_SPEC.md` as the UX contract.
  ...
  - [x] Round 3 drawer interaction local proof slice: collapsed drawer shows only total, signed promo line, and current action state; expanded shadcn bottom Sheet shows item rows/subtotal/promo/shipping/tax/total, closes by handle/Escape/scrim, returns focus, and keeps selected payment action reachable.
    - Evidence: existing focused drawer tests stayed green in the Task 2 verification command, including `aria-expanded`, `aria-controls`, shadcn `Sheet side="bottom"`, handle/Escape/scrim close, focus return, and selected PayPal action in the expanded sheet. Browser metrics for no horizontal overflow, drawer/sticky overlap, and API-backed visual states remain open in the Round 3 evidence helper.
  ...
  - [x] Round 3 selected payment action width slice: selected PayPal and Pay Later provider actions share the same merchant-owned action-slot width in the collapsed drawer, expanded drawer, and desktop/tablet summary where applicable; official provider internals remain untouched.
    - Evidence: failing-first CSS coverage proved the shared checkout selected-payment slot contract was missing, then passed after adding merchant-owned width/fill rules for the desktop summary slot, collapsed sticky action slot, expanded order-sheet payment slot, direct PayPal provider scope child, and PayPal/Pay Later standalone action wrappers. Focused verification passed `npm test -- web/src/styles/global.test.ts` (`19` tests) and `npm test -- web/src/styles/global.test.ts web/src/features/checkout/CheckoutPage.test.tsx` (`55` tests). Browser rect metrics remain open in the Round 3 evidence helper.
    - Inspection standard: CSS tests and browser metrics compare action-slot rects for selected PayPal and selected Pay Later and prove provider custom elements fill the wrapper; official SDK internals, iframes, shadow DOM, and PayPal-rendered content are not restyled.
  ...
  - [ ] Round 3 evidence helper and docs closure: add `tools/round3-checkout-pickup-drawer-evidence.playwright.js` plus `npm run evidence:round3:checkout-pickup-drawer`; update `CART_CHECKOUT_A_PLUS_SPEC.md`, `DESIGN.md`, `IMPLEMENTATION_TASKS.md`, `tracking/todos.md`, `tracking/test-cases.md`, and `tracking/progress.md` with evidence paths and remaining hosted caveats.
    - Inspection standard: local API-backed evidence covers pickup picker open/cancel/confirm, mobile drawer collapsed/expanded/collapsed-again, slow billing, billing failure, real promo discount, selected PayPal, selected Pay Later, selected Card inline, payment-width metrics, header overlap, horizontal overflow, provider counts, create-order request/callback counts, and route-scoped console triage. Hosted smoke remains open until deployed evidence passes.
