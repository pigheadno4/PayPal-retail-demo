...
- [ ] Continue Milestone 16 with Checkout Round 3 pickup and drawer repair using `ROUND3_CHECKOUT_PICKUP_DRAWER_PLAN.md` as the execution handoff and `CART_CHECKOUT_A_PLUS_SPEC.md` as the UX contract.
  ...
  - [ ] Round 3 billing progression slice: Delivery billing opens Shipping method within 250ms after client validation; Pickup billing opens Pickup date within 250ms; pending totals are visible; provider actions remain disabled until latest draft totals are current; failure returns focus/error to Billing.
