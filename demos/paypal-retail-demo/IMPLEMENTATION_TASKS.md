...
- [ ] Continue Milestone 16 with Checkout Round 3 pickup and drawer repair using `ROUND3_CHECKOUT_PICKUP_DRAWER_PLAN.md` as the execution handoff and `CART_CHECKOUT_A_PLUS_SPEC.md` as the UX contract.
  ...
  - [ ] Round 3 real promo activation slice: checkout either calls real backend promo evaluate/apply/remove and displays signed discounts only after success, or hides manual promo activation and shows truthful no-promo status; no fake/inert promo input may render.
    - Inspection standard: API and UI tests prove no discount display without backend discount, signed amount-first display when `discount_minor > 0`, code secondary to amount, and failure/no-promo recovery copy.
