...
- [ ] Polish the checkout selected-payment primary action so choosing PayPal or Pay Later never leaves an ambiguous standalone `Continue` button; the action should focus/click the mounted official provider button or clearly tell the buyer to use the official provider surface.
