...
- `DESIGN.md` owns the visual contract and truth constraints. Do not add fake search, fake preorder economics, fake ratings, fake social auth, unsupported trust claims, or unsupported PayPal behavior to match the reference screenshots.
...
- [x] Keep storefront Pay Later fallback copy out of the rendered subtree once the official PayPal provider is ready; fallback copy remains for SDK config loading/error states.
...
- [x] Fix the follow-up checkout/minicart regressions from GUI review: suppress inline Pickup store cards while the store-picker modal is open, keep the mini-cart `View cart` action visually framed in the portaled Sheet, use the visible black Apple Pay logo in checkout payment rows, and split Pay Later message handling so storefront placements auto-bootstrap official messages while checkout selected Pay Later keeps managed fallback behavior.
