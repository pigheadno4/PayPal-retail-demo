...
- [x] Hide Apple Pay and Google Pay radio rows until device/browser plus provider eligibility is established; feature flags alone are not sufficient eligibility proof. Pending and error states remain absent, Apple requires PayPal `applepay` plus `ApplePaySession.canMakePayments()`, and Google requires PayPal `googlepay` plus `isReadyToPay()`.
  - [x] Keep preselection and selected-action PayPal eligibility amount-aware with the same normalized active fulfillment total; re-probe on Delivery/Pickup switches and draft total recalculation, and remove any selected wallet row/action if eligibility becomes false.
...
- [x] Apply the Cart + Checkout Round 2 density and mobile checkout disclosure refinement from `CART_CHECKOUT_A_PLUS_SPEC.md`: compact cart/title typography, hidden native quantity spinners, form-first mobile checkout, real Delivery/Pickup switch, complete shipping address collection, shipping method section, truthful promo UX, and v5 grabber-only order-details sheet.
  ...
  - [x] Round 2 UI/UX caveat review gate: reconcile the later `ui-ux-pro-max` sub-agent verification verdict `Ready with caveats` into the deploy-quality checklist.
    ...
    - Follow-up evidence: a second read-only `ui-ux-pro-max` sub-agent verification returned `Ready with caveats`: visible Round 2 polish matches the approved virtual mockups. Later local evidence closed the selected-provider browser request/callback caveat for PayPal and Pay Later, the API-backed real discount visual proof with `-$8.55 promo (AUTO10 + BUNDLE8)` in sticky, summary, and Sheet surfaces, selected Pay Later timeout-warning triage, local sticky/fixed overlap for the recorded required targets, helper-owned recalculation/failure visual rows, and post-deploy hosted smoke/required-state sticky-fixed overlap proof.
  ...
  - [x] Round 2 promo/payment truthfulness slice: remove inert or fake promo activation, show truthful no-promo/offer status, display real discounts directly under total in collapsed sticky and inside expanded order details, and keep official provider actions gated by selected method plus settled totals.
  ...
  - [x] Round 2 final verification gate: run focused Cart/Checkout/App/style tests, `npm run typecheck`, `npm run lint`, `npm run format:check`, `git diff --check`, and GUI/browser evidence for cart plus mobile checkout initial, saved, payment-ready, selected-payment, expanded-sheet, collapsed-sheet, and failure/recalculation states.
    ...
    - [x] Focused API-backed provider/discount/sticky rerun evidence: the refreshed `/private/tmp/paypal-retail-cart-checkout-round2-api-backed-20260705/metrics.json` now seeds through `/cart`, removes the homepage route-noise console error, records surface-scoped provider buckets (`checkoutSticky`, `orderSheet`, `inlineCard`, `minicart`, `messageOnly`, `other`), records payment-method attribution from create-order request bodies, records browser callback metrics from the app's SDK callback `console.info` logs, applies real backend promo evaluation/apply for seeded `BUNDLE8`, and records selected `AUTO10 + BUNDLE8` as `-$8.55 promo (AUTO10 + BUNDLE8)` in the sticky summary, order summary, and expanded order-details Sheet. Selected PayPal browser activation creates exactly one `paypal` delivery create-order request delta and one PayPal callback delta; selected Pay Later browser activation creates exactly one `paylater` delivery create-order request delta and one Pay Later callback delta; blocked/no-method/card/overlay/focused-input rows stay at zero request/callback deltas. The latest sticky-clearance rerun records `stickyOverlapTargets[].occludedByStickyCount === 0` for the recorded selected Card inline form, focused billing street input, provider buttons/messages, Sheet controls, footer content, validation errors, mobile menu controls, minicart controls, sign-in dialog controls, and order-details controls. The 2026-07-06 Pay Later timeout telemetry fix closes the selected Pay Later warning-triage item by reporting expected timeout fallback as structured `console.info` instead of `console.warn`; real SDK/fetch/content-application failures still log as errors. Post-deploy hosted smoke now closes the remaining hosted sticky/fixed overlap proof.
    - [x] Helper-owned recalculation/failure visual evidence: committed `tools/round2-checkout-readiness-evidence.playwright.js` and `npm run evidence:round2:checkout-readiness` to generate route-intercepted local browser rows without buyer-visible QA flags. Evidence under `/private/tmp/paypal-retail-round2-checkout-readiness-evidence/` includes `checkout-recalculating-readiness-390.png`, `checkout-failed-readiness-390.png`, `checkout-shipping-save-failed-390.png`, and `metrics.json`. The helper fails if expected displayed labels, zero provider counts, zero create-order request/callback deltas, zero horizontal overflow, zero sticky occlusion, or console allowlist rules regress. The recalculating and failed readiness rows assert visible readiness copy, displayed total/promo/shipping/tax labels, and no row-scoped console warnings/errors. The shipping-save failure row asserts the retryable Shipping address alert, expected displayed labels, zero provider/create-order/callback counts, exactly one intercepted shipping-address `500`, exactly one matching checkout-draft app failure log, and zero unrelated console errors. Post-deploy hosted smoke closes the hosted sticky/fixed overlap proof for the deployed asset.
    - [x] Local hosted-smoke helper evidence: committed `tools/round2-hosted-checkout-smoke.playwright.js` and `npm run evidence:round2:hosted-smoke` to generate the deploy-smoke matrix against either the current Playwright page origin or Render. Local Vite/API evidence at `/private/tmp/paypal-retail-round2-local-smoke-evidence.json` passed `27` rows with `failedRows: []`, covering `/cart` and `/checkout` at `320`, `375`, `390`, `768`, `1024`, `1280`, and `1440`, plus 390px billing, shipping method, payment-ready no-method, selected PayPal, expanded order details, collapsed focus return, selected Pay Later, selected Card, focused input, footer clearance, mobile menu, minicart, and sign-in dialog states. The helper records screenshot paths, route-scoped console/response issues, horizontal overflow, sticky occlusion, provider buckets by surface, create-order request/callback baselines/deltas/cumulative counts, grabber ARIA/rects, order-sheet state, focused element, and overlay suppression. Local footer clearance now reports zero sticky occlusion after adding footer bottom reserve while the checkout sticky summary exists.
    - [x] Hosted post-deploy smoke evidence: pre-deploy Render health returned `200` with debug ID `dbg_ba8fd084a906`, but the deployed index still served `/assets/index-CuNnoNV5.js` and `/assets/index-CajkqOhp.css`; the new hosted smoke against that old build failed on `cart-first-pass-320` cart-restore fetch noise and `checkout-footer-clearance-390` with `sticky occluded 7 independent targets`. After deployment, Render health returned `200` with debug ID `dbg_d40682632c40`, live assets changed to `/assets/index-uhOMu6vW.js` and `/assets/index-DdbX4s-t.css`, and `/private/tmp/paypal-retail-round2-hosted-smoke-after-deploy.json` passed `27` rows with `failedRows: []`. The deployed smoke records zero cart-first-pass console/response issues, zero footer sticky occlusion, and no checkout sticky summary while mobile menu, minicart, or sign-in dialog overlays are open.
- [ ] Continue Milestone 16 with Checkout Round 3 pickup and drawer repair using `ROUND3_CHECKOUT_PICKUP_DRAWER_PLAN.md` as the execution handoff and `CART_CHECKOUT_A_PLUS_SPEC.md` as the UX contract.
  - [x] Round 3 pickup picker layer local implementation slice: store picker layer is explicitly above `.site-header`; custom modal layer now uses `z-index: 50` while `.site-header` remains `z-index: 40`, matching shadcn Dialog/Sheet layer semantics instead of overriding them.
    - Evidence: focused style tests prove picker layer order with `npm test -- web/src/features/checkout/CheckoutPage.interactions.test.tsx web/src/styles/global.test.ts`. Browser metrics for 320, 390/414, and 1440 remain open in the Round 3 evidence helper.
  - [x] Round 3 pickup cancel fallback local implementation slice: canceling or dismissing the picker before store confirmation returns to Pickup location for re-submit, keeps Store selection dormant, hides inline store cards, clears only pending modal selection, and commits no store.
    - Evidence: interaction tests cover submit ZIP -> modal open -> cancel -> Pickup location active, no inline store cards, and no selected store. Browser evidence repeats the path in the Round 3 evidence helper.
  - [x] Round 3 mobile drawer ownership local implementation slice: mobile checkout removes duplicated full order-detail content from the main page when the bottom drawer is present, while desktop/tablet summary remains visible.
    - Evidence: failing-first mobile test `makes the mobile bottom drawer the only order detail surface` first failed because the complementary `Order summary` card was still present, then passed after unmounting `CheckoutSummary` on mobile when the sticky drawer is active. The reviewer-requested fallback regression proves the full mobile summary returns when `suppressMobileStickySummary` intentionally disables the sticky drawer. Focused verification passed `npm test -- web/src/features/checkout/CheckoutPage.test.tsx web/src/features/checkout/CheckoutPage.interactions.test.tsx web/src/styles/global.test.ts` (`75` tests). Browser-matrix evidence remains open in the Round 3 evidence helper.
  - [x] Round 3 drawer interaction local proof slice: collapsed drawer shows only total, signed promo line, and current action state; the drawer opens from the summary surface/passive handle without a separate expand button; expanded shadcn bottom Sheet shows item rows/subtotal/promo/shipping/tax/total, closes by handle/Escape/scrim, returns focus, and keeps selected payment action reachable.
    - Evidence: existing focused drawer tests stayed green in the Task 2 verification command, including `aria-expanded`, `aria-controls`, shadcn `Sheet side="bottom"`, handle/Escape/scrim close, focus return, and selected PayPal action in the expanded sheet. Follow-up CSS coverage now distinguishes the passive visual handle from the real summary trigger target so the handle no longer reads as a button. Browser metrics for no horizontal overflow, drawer/sticky overlap, and API-backed visual states remain open in the Round 3 evidence helper.
  - [x] Round 3 billing progression local implementation slice: Delivery billing opens the Shipping options/method section within 250ms after client validation; Pickup billing opens Pickup date within 250ms; pending totals/recalculating state is preserved while the draft update settles; dependent submit actions stay disabled until upstream draft data is reconciled; provider actions remain disabled until latest draft totals are current; and slow-save failures return focus/error to Billing.
    - Evidence: failing-first interaction tests covered slow Delivery billing, slow Pickup billing, literal focus return to Billing, Delivery billing failure rollback, and Pickup billing failure rollback. After the hosted helper exposed a fast-click race that submitted static fallback `ship_standard` to Render and produced `PATCH /api/checkout/drafts/:id/shipping-option` `500`, `CheckoutPage.interactions.test.tsx` now also proves the optimistic Shipping options section can render while Billing recalculates but its `Submit shipping option` button is disabled. Focused verification passed `npm test -- web/src/features/checkout/CheckoutPage.interactions.test.tsx server/tests/evidenceScripts.test.ts` (`25` tests), `npm run typecheck -- --pretty false`, `npm run lint`, `npm run format:check`, and `git diff --check`. Browser/API-backed timing and visual proof remains open in the Round 3 evidence helper.
    - Inspection standard: interaction tests simulate slow and failed billing updates for Delivery and Pickup and record timing, active section, readiness state, displayed total/promo/shipping/tax labels, and disabled dependent submit state while upstream draft updates are saving/recalculating.
  - [x] Round 3 real promo activation slice: checkout either calls real backend promo evaluate/apply/remove and displays signed discounts only after success, or hides manual promo activation and shows truthful no-promo status; no fake/inert promo input may render.
    - Evidence: added typed frontend helpers for backend `evaluate`, `apply`, and `remove` promo routes, plus App-level orchestration that evaluates and applies recommended discounts only after a successful checkout draft update with no already-selected promo. Zero-discount/rejected evaluations do not apply, network failures return a non-discount state, and the buyer summary updates only from the apply response. `CheckoutPage.test.tsx` now also proves signed amount-first promo text in the collapsed mobile drawer and expanded order-details sheet. Focused verification passed `npm test -- web/src/features/checkout/CheckoutPage.test.tsx` (`36` tests) and `npm test -- web/src/features/checkout/checkoutDraftApi.test.ts web/src/features/checkout/CheckoutPage.test.tsx web/src/app/App.interactions.test.tsx` (`109` tests). Browser/API-backed discount visual proof remains open in the Round 3 evidence helper.
    - Inspection standard: API and UI tests prove no discount display without backend discount, signed amount-first display when `discount_minor > 0`, code secondary to amount, truthful no-promo state when activation is rejected/zero/failed, and no fake/inert manual promo input.
  - [x] Round 3 selected payment action width slice: selected PayPal, Pay Later, Apple Pay, Google Pay, and Venmo provider actions share the same merchant-owned action-slot width and height in the collapsed drawer, expanded drawer, and desktop/tablet summary where applicable; official provider internals remain untouched.
    - Evidence: failing-first CSS coverage proved the shared checkout selected-payment slot contract was missing, then passed after adding merchant-owned width/fill rules for the desktop summary slot, collapsed sticky action slot, expanded order-sheet payment slot, direct PayPal provider scope child, PayPal/Pay Later standalone action wrappers, and wallet action wrappers. Compact sticky/order-sheet slots suppress Pay Later financing-message height so Pay Later matches Venmo/Apple Pay/Google Pay action geometry. Focused verification passed `npm test -- web/src/styles/global.test.ts` and the combined checkout/style suite; browser rect metrics remain open in the Round 3 evidence helper.
    - Inspection standard: CSS tests and browser metrics compare action-slot rects for selected PayPal, Pay Later, Apple Pay, Google Pay, and Venmo and prove provider custom elements fill the wrapper; official SDK internals, iframes, shadow DOM, and PayPal-rendered content are not restyled.
  - [x] Round 3 preselected Pickup store summary cleanup slice: logged-in/preselected Pickup starts at Billing and the store summary has only the change-store edit affordance, with no redundant `Continue with this store` action.
    - Evidence: checkout tests prove preselected Pickup renders the store summary, inventory status, Billing address editor, and change-store control while omitting the redundant continuation button. Browser evidence remains open in the Round 3 evidence helper.
  - [x] Round 3 App-level payment readiness regression slice: after picker/drawer/promo changes, App-level tests prove no-method, recalculating, failed, focused input, open pickup picker, open expanded drawer, open mobile menu/minicart/sign-in, selected Card, and stale/missing draft states produce zero PayPal create-order requests/callbacks; selected PayPal and selected Pay Later with settled/current totals produce exactly one method-attributed request/callback when activated from collapsed and expanded drawer states.
    - Evidence: `App.checkout-paypal-capture.test.tsx` now covers stale/syncing readiness, missing selected-payment draft, collapsed mobile drawer activation for selected PayPal and Pay Later, expanded drawer activation for Pay Later, and selected-PayPal pickup picker suppression after returning upstream from Pickup payment. Existing App coverage remained green for no method, recalculating, failed, focused upstream input, mobile menu, minicart Sheet, sign-in dialog, selected Card inline/no sticky provider behavior, expanded PayPal drawer activation, and open/close drawer without payment. Focused verification passed `npm test -- web/src/app/App.checkout-paypal-capture.test.tsx web/src/app/App.interactions.test.tsx` (`86` tests). Browser/API-backed request and callback deltas remain open in the Round 3 evidence helper.
    - Inspection standard: App tests assert zero/one create-order request counts and zero/one SDK callback counts by blocked/allowed state; browser evidence must still record matching request/callback baselines/deltas for blocked and allowed states.
  - [ ] Round 3 evidence helper and docs closure: add `tools/round3-checkout-pickup-drawer-evidence.playwright.js` plus `npm run evidence:round3:checkout-pickup-drawer`; update `CART_CHECKOUT_A_PLUS_SPEC.md`, `DESIGN.md`, `IMPLEMENTATION_TASKS.md`, `tracking/todos.md`, `tracking/test-cases.md`, and `tracking/progress.md` with evidence paths and remaining hosted caveats.
    - Evidence: helper and npm script are now added, and `server/tests/evidenceScripts.test.ts` statically locks the required command, row IDs, metric names, checkout-readiness waiting, and pickup-store commit compatibility. Initial hosted Render execution exposed two helper drifts (product-name readiness and optional pickup confirmation) plus one real optimistic-submit race; the helper is now patched for the drifts, and runtime/tests now block the fast-click shipping-option race locally. Full API-backed helper execution remains open until the patched code is deployed or a configured local app is available.
    - Inspection standard: local API-backed evidence covers pickup picker open/cancel/confirm, preselected Pickup store summary without redundant continuation, mobile drawer collapsed/expanded/collapsed-again from the summary surface/passive handle, slow billing, billing failure, real promo discount, selected PayPal, selected Pay Later, selected Apple Pay, selected Google Pay, selected Venmo, selected Card inline, payment-width/height metrics, header overlap, horizontal overflow, provider counts, create-order request/callback counts, and route-scoped console triage. Hosted smoke remains open until deployed evidence passes.
...
- [x] Round 4 planning/mockup gate: use `ui-ux-pro-max` design-system and targeted retrieval, keep `ROUND4_AUTH_MINICART_CHECKOUT_POLISH_PLAN.md` as the source of truth, keep `mockups/round4-auth-minicart-checkout-polish.html` as the visual reference, and do not start runtime edits until tracking/test rows are present.
- [x] Round 4 typography and favicon foundation: scoped softer transaction tokens remain local to Auth/Minicart/Checkout-owned surfaces; the strict browser report proves 320px fitting, favicon `200`, zero issue rows, and `65` scoped contrast samples with a `4.60:1` minimum.
- [x] Round 4 email sign-in modal polish: the centered visible-label form, `inputmode="email"`, full-width `Continue`, validation, focus, close, and focus-return behavior have local code proof plus browser rows at 320/390/1440 with `0px` input/action width delta.
- [x] Round 4 password/register modal polish: inline eye/eye-off toggles, `Edit email`, full-width CTAs, first-field focus, current/new-password autocomplete, terms/password errors, loading, no fake social auth, session persistence, and cart merge behavior have local code proof plus 320/390/1440 browser evidence.
- [x] Round 4 minicart polish: shadcn Sheet behavior, product-first hierarchy, measured line clamp, 44px named quantity controls, Checkout/View cart hierarchy, Pay Later placement, sticky suppression, and focus return have local code proof plus 320/390/1440 browser evidence.
- [x] Round 4 checkout payment/order-sheet polish: buyer-facing copy, compact safeguards/readiness, visible neutral grabber, top padding under 24px, Escape/scrim/handle focus return, provider surface ownership, official `paypal-button` / `paypal-pay-later-button` / `paypal-hosted-card-field` nodes, and inherited exactly-once activation semantics have local code and browser proof.
- [x] Round 4 pickup inventory row polish: shared modal/inline markup, two-line item names, narrow status reflow, shrink-safe heading/badge layout, desktop right alignment, and text-labelled full/partial/sold-out states have local code proof plus picker/preselected browser rows at 320/390/1440 with zero overflow/header coverage.
- [x] Round 4 local evidence helper and correction review gate: `31` required rows and `40` quality-95 JPEGs pass under `/private/tmp/paypal-retail-round4-local-auth-minicart-checkout-evidence/` with a review-visible ignored copy under `.playwright-cli/round4-local-auth-minicart-checkout-evidence/`. The report proves Auth email/password/register initial and return focus, open Minicart, checkout payment/order-sheet/readiness/safeguards, pickup picker/preselected inventory, zero console/response/overflow/overlap/suspicious-pixel issues, favicon `200`, `127` direct contrast samples at `4.60:1` minimum, named 44px quantity controls, visible grabber, three close methods, exact provider buckets, placement-scoped official PayPal/Pay Later/Card Fields nodes, runtime-resolved selected actions, and dedicated visible full/partial/empty pickup screenshots at 320/390/1440. Fresh `npm run verify` passes `606` tests across `69` files plus typecheck/lint/format; local and hosted read-only correction reviews found no unresolved P0/P1/P2 findings.
- [x] Round 4 hosted Render correction gate: deployed commit `87f7ec96`, reran the strict helper against Render assets `index-C1Q5eUuT.js` and `index-Cw49jF97.css`, and closed the route/state review. The persisted report contains `31` rows and `40` JPEGs with `missingRows: []`, `failedRows: []`, `4.60:1` minimum contrast, 44px minimum measured target, and `0.0027` maximum near-black ratio. It proves runtime-resolved hydrated provider geometry, pending fallback replacement, nonempty checkout contrast samples, one visible branded method name, one-line 320px pickup confirmation, and the corrected Auth/Minicart/Card presentation. A fresh read-only `ui-ux-pro-max` reviewer verified metrics SHA1 `436b1c6b9b53ad47e3b444966deb740959ed7d6f`, inspected all `40` hosted JPEGs, and returned PASS with no P0/P1/P2 findings.
  - [x] Hosted origin rename/asset preflight repair: active Round 2/3/4 helpers classify any HTTPS `*.onrender.com` origin as hosted; Round 2 defaults to `https://paypal-retail-demo.onrender.com`; Round 4 runs `HEAD` checks for `/assets/paypal-logos/applepay-default.svg` and `/assets/popmart/products/blind-boxes-1-1.png` before browser evidence. Live probes on the renamed origin returned `200` for health and both assets, and a fresh browser load reported no console warnings/errors.
  - [x] Hosted runner execution repair and strict evidence pass: the wrapper preserves Playwright `### Error` output, retries one hosted `net::ERR_ABORTED` navigation, waits for complete guest-cart and Minicart readiness, and applies the 90-second hosted budget to the intercepted Pickup request without retrying the mutating `PATCH`. The final correction run against `index-C1Q5eUuT.js` persisted `31` rows and `40` JPEGs with `missingRows: []`, `failedRows: []`, `4.60:1` minimum contrast, 44px minimum measured target, and `0.0027` maximum near-black ratio; independent hosted visual review passed.
  - [x] Hosted visual-review correction local proof: visible Auth/Minicart images now use a shared buyer-safe failure fallback, the helper waits for viewport-visible images to finish and decode before capture, selected Card shows one concise sticky instruction instead of a contradictory disabled action, and password/register email summaries use single-line ellipsis without shrinking the 44px `Edit email` target. The refreshed local matrix passes all `31` rows and `40` JPEGs with `missingRows: []`, `failedRows: []`, `4.60:1` minimum contrast, 44px minimum measured target, and `0.0027` maximum near-black ratio.
  - [x] Fresh read-only local review inspected all `40` checksum-locked current-code JPEGs and found no P0, P1, or P2 findings; the subsequent independent hosted review inspected all `40` deployed captures and returned the same PASS verdict.
- [x] Correct the four 2026-07-16 Render stateful-verification regressions with failing-first coverage: postal-scoped tax fallback when PayPal omits county, authoritative empty resumed shipping options, DELETE-at-zero minicart reconciliation, and UUID-safe Payment Diagnostics lookup.
- [x] Correct the first decline review: isolate missing-county tax fallback to PayPal callbacks, rank only verified scopes with longest-prefix and ambiguity protection, serialize cart quantity writes, suppress stale response repaint, and restore canonical cart state after failure.
- [x] Correct the second decline review: replace the quantity-only queue with one cart-operation coordinator spanning quantity, PDP add, login merge, payment refresh, and post-capture reload; prove checkout/express refresh waits for pending writes and stale PDP add responses cannot repaint newer quantity intent.
