...
- [ ] Continue Milestone 16 with Checkout Round 3 pickup and drawer repair using `ROUND3_CHECKOUT_PICKUP_DRAWER_PLAN.md` as the execution handoff and `CART_CHECKOUT_A_PLUS_SPEC.md` as the UX contract.
  ...
  - [ ] Round 3 selected payment action width slice: selected PayPal and Pay Later provider actions share the same merchant-owned action-slot width in the collapsed drawer, expanded drawer, and desktop/tablet summary where applicable; official provider internals remain untouched.
    - Inspection standard: CSS tests and browser metrics compare action-slot rects for selected PayPal and selected Pay Later and prove provider custom elements fill the wrapper.
