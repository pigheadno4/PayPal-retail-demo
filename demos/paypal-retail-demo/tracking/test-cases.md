...
- [ ] Category + PDP Reference Polish V5 preaudit verifies live Render baseline before runtime edits: Category, released PDP, and unreleased PDP screenshots/metrics are captured at 1440, 1280, 1024, 768, 390, and 320 widths, and the evidence records first-product depth, Pay Later placement, PDP tab dimensions, support-card footprint, unreleased payment-gating state, page-level horizontal overflow, and console warnings/errors.
...
- [ ] Category V5 filters and sort verify primary controls sit above the grid, active filters remain visible as chips, secondary filters use a shadcn `Sheet` or compact popover, sort remains scannable, disabled pickup filtering includes a buyer-readable reason, and `q`, category, release status, price, availability, pickup context, and sort state survive reload/back/forward.
...
- [ ] Category V5 card states verify sale, released, coming-soon, and unavailable cards are visually distinct: sale and coming-soon badges do not overlap, coming-soon media is muted/desaturated with text label, unreleased cards cannot start cart/checkout/payment, lazy media/skeleton boxes prevent layout jump, and no old mock imagery flashes during loading.
- [ ] PDP V5 unreleased gating verifies unreleased PDPs hide Add to cart, PayPal, Pay Later, and delivery express frames/messages, render a disabled coming-soon/not-released action with release context, hide reviews/review summary, and keep product inspection/facts/story/shipping information accessible.
- [ ] PDP V5 released purchase rail verifies purchasable PDPs keep official Pay Later directly under price, compact the support/trust cards into a lower-footprint band/accordion/chip group, preserve supported purchase-option behavior, and reduce the desktop blank gap below the gallery without adding fake status panels or unsupported claims.
- [ ] PDP V5 reviews verify released products with real review data show a rating/review summary near the title and a review preview/summary in the lower detail area; score/count come from seeded/submitted reviews, no fake rating renders when data is absent, and review cards remain discoverable from the Customer reviews detail section.
- [ ] PDP V5 detail navigation verifies desktop shadcn/Radix tabs have clear active state, no vertical scrollbar artifact, 44px+ triggers, keyboard/click activation, and inactive panels hidden from the accessibility tree; mobile uses a constrained horizontal rail or accordion/collapsible pattern with no clipped inaccessible right-side tabs and no page-level horizontal overflow.
- [ ] PDP V5 mobile sticky purchase verifies a released-product sticky Add to cart bar appears only when the main CTA is out of view, mirrors selected option/quantity/price, uses the same add-to-cart handler, reserves safe-area bottom padding, avoids covering PayPal messages/tabs/footer controls, disappears when the main CTA is visible, and does not expose payment actions for unreleased products.
