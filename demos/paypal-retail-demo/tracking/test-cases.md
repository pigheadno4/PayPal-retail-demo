...
- [x] PDP V5 focused unit/static coverage verifies unreleased PDPs branch away from the released purchase rail, render a shadcn-backed `product-release-preview` card with disabled `Coming soon` action and release context, omit Add to cart, PayPal frame/buttons, Pay Later message/button, payment/trust support cards, review summary/cards/social-proof cards, enabled notify/email capture, and sticky purchase candidates, while keeping breadcrumb/gallery/title/status/vendor/price/facts/detail tabs/shipping/Q&A inspection content accessible. Regression coverage also proves accidental upstream social-proof data cannot reopen the reviews tab before release, and the CSS contract keeps PayPal action sizing on merchant-owned wrappers rather than PayPal SDK custom-element selectors.
