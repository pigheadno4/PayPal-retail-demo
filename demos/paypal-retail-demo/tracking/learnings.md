...
- A cross-surface lifecycle evidence row needs fixture intersection, not two independently valid datasets: at least one signed-in Account order must also be actionable in Admin. Detect and report the empty intersection before mutation; never relabel a guest order, force a completed order backward, or claim webhook neutrality without a real lifecycle transition.
- A deterministic seed used for mutation evidence must reconcile mutable child history before canonical upserts. Delete lifecycle events only for order IDs explicitly owned by the seed dataset, then restore canonical events; this keeps reruns repeatable without sweeping unrelated merchant data.
- A successful browser response event is not proof that React committed the response body. Cross-surface evidence should bind the API object identity and status first, then wait for the exact rendered identity/status/note state before reading metrics.
- Hosted SPA evidence should not use `DOMContentLoaded` as product readiness when blocking third-party wallet scripts are present. Wait for navigation `commit`, then prove the exact route-owned auth/UI/API state; keep retry limited to idempotent page GET/reload and never replay a mutation.
- A hosted API can be correct but slower than a generic interaction timeout. Register the exact expected response before navigation, validate its HTTP status and canonical object identity, use a bounded hosted-specific readiness budget supported by measured evidence, and only then wait for the corresponding UI commit.
- Evidence that must work against the currently deployed build cannot depend on a DOM readiness marker introduced only in the unshipped branch. When browser storage can change the branch, use an isolated context to make the precondition deterministic and join the resulting UI/API waiters.
- Repeatability claims need independently retained artifacts. Validate run IDs, reject path-like values, and use exclusive creation for named metrics so a later pass cannot overwrite the evidence being cited.
- Transient loading evidence should hold and later fulfill the untouched real response, match the exact list endpoint rather than a broad path prefix, capture the frame before release, require the exact expected status and final settled state, and unregister routing in `finally`.
- Radix `asChild` composition can replace a child's `data-slot` attribute. Style and test critical composed controls through an explicit semantic class when the rendered slot identity matters, then confirm the computed target in browser evidence.
- Browser evidence runners must keep secrets out of child argv, use short randomized session names that stay below macOS Unix-socket limits, and close authenticated sessions in `finally`. If the browser helper cannot inherit environment variables or read a stdin filename, use a mode-0600 one-purpose helper, delete it in `finally`, and track forced-kill residue as an explicit residual risk instead of moving the secret into argv. Evidence should capture warnings as well as errors, exact-match any external allowlist with surface/location qualification, reject absent numeric totals, require visible or explicit-empty datasets without fallback values, scan diagnostics for forbidden fields, and recheck scroll-discovered occlusion with target center and edge samples at an accessible centered position.
...
- Kickoff requirements can still be lost when milestone tasks compress them into broad labels. For complex buyer flows, each user-visible promise needs a named acceptance test and a live-state/browser evidence gate before the milestone can close.
...
- Pay Later message QA must verify buyer-visible content or a visible fallback, not just a mounted `paypal-message` host or provider `ready` state. A passive SDK message host can reserve layout space while rendering no text, shadow content, or iframe.
