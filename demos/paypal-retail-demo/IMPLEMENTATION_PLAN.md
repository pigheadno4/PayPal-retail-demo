...
- When checkout payment actions are not yet eligible or active, fill reserved merchant-owned payment space with buyer-readable state copy instead of an empty panel; keep official PayPal-rendered controls untouched once they mount.
...
- Instrument PayPal create-order starts, prepared request state, gateway handoff, and failures with structured frontend/backend diagnostics. Logs may include route stage, method/source/kind, cart/draft/session/order IDs, amount/currency, generated debug ID, and safe booleans such as `hasCartClientSecret`; logs must never include bearer tokens, cart secrets, PayPal client secrets, OAuth access tokens, or raw buyer PII.
