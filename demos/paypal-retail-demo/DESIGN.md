...
- Wallet preselection eligibility contract (2026-07-12): Apple Pay and Google Pay radio rows start absent, not disabled or loading. Checkout mounts nonvisual method-scoped PayPal eligibility probes with the same normalized active fulfillment amount used by the selected wallet action, and re-probes when fulfillment mode or its total changes. Apple Pay becomes visible only when PayPal reports `applepay` eligible and `window.ApplePaySession.canMakePayments()` returns `true`. Google Pay becomes visible only when PayPal reports `googlepay` eligible and Google `PaymentsClient.isReadyToPay()` returns `{ result: true }` for PayPal's formatted allowed payment methods. Probe errors resolve to ineligible, never create an order, and never expose a buyer-facing fallback row. Venmo keeps its existing market/provider behavior.
...
- Preselected Pickup store summaries are editable summaries only: do not show a redundant `Continue with this store` action, and make Billing the next active buyer task.
- Collapsed mobile drawer shows only total, signed promo amount when present, and the current payment action state. It does not show item descriptions, thumbnails, explanatory payment text, trust copy, selected-method label rows, or a separate expand button; the summary surface/passive handle opens the sheet.
...
- 2026-07-07 local implementation note: checkout draft updates now evaluate and apply recommended backend promos through typed frontend route helpers; zero-discount/rejected evaluations stay no-promo and network failures do not fabricate a discount. The buyer-facing signed amount is reconciled only from the backend apply response, with focused local proof for collapsed drawer, expanded order-details sheet, and App summary display.
- Selected PayPal, Pay Later, Apple Pay, Google Pay, and Venmo actions should occupy the same merchant-owned action slot width and height in the collapsed drawer, expanded drawer, and desktop/tablet summary where applicable. Official provider internals remain untouched.
- 2026-07-07 local implementation note: selected PayPal, Pay Later, Apple Pay, Google Pay, and Venmo now share the same merchant-owned checkout action slot width/fill contract across desktop summary, collapsed drawer, and expanded order sheet through wrapper CSS only; compact sticky/order-sheet Pay Later financing copy is suppressed inside the selected action slot so Pay Later does not become taller than wallet actions. Browser rect evidence remains in the Round 3 helper.
- 2026-07-07 evidence helper note: `npm run evidence:round3:checkout-pickup-drawer` is registered and the helper records the required drawer/picker/payment metrics, including selected-action rect parity against Venmo, passive drawer trigger geometry, picker/header overlap, preselected-store continuation absence, billing transition timing, provider buckets, create-order deltas, and route-scoped console triage. The helper is statically verified; API-backed screenshot/metrics output is still required before deploy-quality closure.
- Round 3 deploy-quality evidence must include pickup picker open/cancel/confirm, preselected Pickup store summary without continuation, mobile drawer collapsed/expanded/collapsed-again from the summary surface/passive handle, slow and failed billing submit, real promo discount, selected PayPal, selected Pay Later, selected Apple Pay, selected Google Pay, selected Venmo, selected Card inline, equal selected-action rects, and header/drawer overlap metrics at 320, 390/414, and 1440.
