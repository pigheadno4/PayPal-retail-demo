...
export function quoteRequiresReplacement(
  current: StoredQuote,
  draft: GoMonthlyQuoteDraft,
  now: Date,
): boolean {
...
export async function replaceCurrentQuote(input: Readonly<{
  ...
  return {
    review: toCheckoutReview(result.quote),
    replacementCreated: result.kind === "replaced",
  };
}
...
export async function requireCurrentQuoteForPayment(input: Readonly<{
  ...
  const current = await input.repository.findOwnedQuote(
    input.accountId,
    input.intentId,
    input.quoteId,
  );
