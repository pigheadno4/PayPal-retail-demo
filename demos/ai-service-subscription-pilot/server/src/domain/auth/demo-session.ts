...
function signature(value: string, secret: string): string {
  return createHmac("sha256", key(secret, "demo-session-cookie-v1"))
    .update(value)
    .digest("base64url");
}
...
export function verifySignedDemoSession(
  ...
  if (!publicId || !rawToken || !expiresRaw || !supplied) {
  ...
  return { publicId, tokenHash: hashSessionToken(rawToken), expiresAt } as const;
}
