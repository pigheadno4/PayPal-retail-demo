...
import { decryptDemoOtp, encryptDemoOtp, sessionHashesMatch, verifySignedDemoSession } from "@/server/auth/demo-session";
...
export async function processSendEmailHook(input: Readonly<{
  ...
  try {
    payload = new Webhook(input.hookSecret.replace(/^v1,whsec_/, "")).verify(input.rawBody, input.headers) as HookPayload;
  } catch {
    throw new HookRejectedError();
  }
  if (!payload?.user?.email || !/^\d{6}$/.test(payload?.email_data?.token ?? "")) throw new HookRejectedError();

  const session = await input.repository.findByAlias(payload.user.email);
  if (session && payload.user.email.endsWith("@test")) {
    const now = input.clock();
    if (session.expiresAt.getTime() <= now.getTime() || session.consumedAt) return;
...
export async function retrieveDemoOtp(input: Readonly<{
  ...
  if (!session || !sessionHashesMatch(session.tokenHash, proof.tokenHash) || session.consumedAt || !session.otpCiphertext || !session.otpExpiresAt) {
    throw new DemoOtpUnavailableError();
  }
