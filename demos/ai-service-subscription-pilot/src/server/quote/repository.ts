...
import { quoteRequiresReplacement, type QuoteRepository, type StoredQuote } from "@/server/quote/service";
import { GO_MONTHLY_SEATTLE_FIXTURE } from "@/server/quote/go-monthly-seattle";
...
export function mapQuoteRow(row: QuoteRow): StoredQuote {
  return {
    ...
    taxBasisPoints: Number(row.tax_basis_points) as 1055, taxCents: Number(row.tax_cents), totalCents: Number(row.total_cents), pricingVersion: row.pricing_version,
    ...
    timeZone: row.time_zone as "America/Los_Angeles", locationKey: GO_MONTHLY_SEATTLE_FIXTURE.locationKey,
  };
}
  ...
  async replaceOwnedQuoteAtomically(input: Readonly<{
    ...
    return sql.begin(async (tx) => {
      ...
      const rows = await tx<QuoteRow[]>`
        ...
        values
          (${randomUUID()}, ${current.intentInternalId.toString()}, 'USD', ${input.draft.baseCents}, ${input.draft.promotionCents}, ${input.draft.taxableSubtotalCents},
           ${input.draft.taxBasisPoints}, ${input.draft.taxCents}, ${input.draft.totalCents}, ${input.draft.pricingVersion}, ${input.draft.taxVersion},
           ${new Date(input.draft.issuedAt)}, ${new Date(input.draft.expiresAt)}, ${new Date(input.draft.renewsAt)}, ${new Date(input.draft.allowanceResetsAt)},
           ${input.draft.timeZone}, ${current.internalId.toString()})
