...
export function createGoMonthlyQuote(clock: () => Date = () => new Date()): GoMonthlyQuoteDraft {
  const issuedAt = clock();
  const expiresAt = new Date(issuedAt.getTime() + GO_MONTHLY_SEATTLE_FIXTURE.quoteLifetimeMinutes * 60_000);
  const renewsAt = new Date(issuedAt);
  renewsAt.setUTCMonth(renewsAt.getUTCMonth() + GO_MONTHLY_SEATTLE_FIXTURE.renewalMonths);
  const taxableSubtotalCents = GO_MONTHLY_SEATTLE_FIXTURE.baseCents + GO_MONTHLY_SEATTLE_FIXTURE.promotionCents;
  const taxCents = Math.round(taxableSubtotalCents * GO_MONTHLY_SEATTLE_FIXTURE.taxBasisPoints / 10_000);
